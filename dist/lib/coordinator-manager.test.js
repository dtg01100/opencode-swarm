...
describe('CoordinatorManager', () => {
    ...
    describe('initSwarmForTask', () => {
        it('should create swarm with planner task and agent', async () => {
            const client = createMockClient();
            manager.setClient(client);
            const { swarmId, plannerAgentId } = await manager.initSwarmForTask('Build user authentication', 'root-session-1');
            expect(swarmId).toBeDefined();
    ...
    describe('spawnWorkerAgent', () => {
        it('should throw when no active swarm', async () => {
            await expect(manager.spawnWorkerAgent({
                role: 'coder',
                taskId: 'task-1',
    ...
    describe('reportProgress', () => {
        it('should throw when no active swarm', async () => {
            await expect(manager.reportProgress({
                agentId: 'agent-1',
                progress: 0.5,
            })).rejects.toThrow('No active swarm');
        });
    ...
    describe('completeCurrentAgent', () => {
        it('should throw when no active swarm', async () => {
            await expect(manager.completeCurrentAgent('agent-1', 'Done!')).rejects.toThrow('No active swarm');
        });
    ...
    describe('failAgent', () => {
        it('should throw when no active swarm', async () => {
            await expect(manager.failAgent('agent-1', 'Network error')).rejects.toThrow('No active swarm');
        });
    ...
    describe('broadcast', () => {
        it('should throw when no active swarm', async () => {
            await expect(manager.broadcast('Hello agents')).rejects.toThrow('No active swarm');
        });
    });
    describe('abortSwarm', () => {
        it('should throw when no active swarm', async () => {
...
describe('Coordinator', () => {
    ...
    describe('handleHandoff', () => {
        it('should write handoff file and log event', async () => {
            ...
            await coordinator.handleHandoff(data);
            ...
            const events = coordinator.db.getEvents();
            const handoffEvent = events.find((e) => e.type === 'handoff');
            expect(handoffEvent).toBeDefined();
            expect(handoffEvent.data.taskId).toBe('task-1');
        });
