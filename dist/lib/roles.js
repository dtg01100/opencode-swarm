...
export function mergeCustomRoles(customRoles) {
    for (const [role, customDef] of Object.entries(customRoles)) {
        if (customDef) {
            const defaultDef = DEFAULT_ROLES[role];
            if (defaultDef) {
                DEFAULT_ROLES[role] = {
                    ...defaultDef,
                    ...customDef,
