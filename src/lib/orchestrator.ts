...
export class Orchestrator {
  private manager: CoordinatorManager;
  ...
  constructor(manager: CoordinatorManager = coordinatorManager, basePath = '.opencode/swarm') {
  ...
  private scheduleOnce = async () => {
    ...
    if (!depsOk) return; // wait until later
    ...
    try {
      ...
      if (coordinator) {
        ...
        if (runningAgents >= maxConc) useNew = true;
      } else {
        useNew = true;
      }
